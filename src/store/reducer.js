  ...
  return updateObject(state, {
    allCourseData: state.allCourseData.map(course => {
      if (course.isSelected) {
        ...
        updatedCategories = updatedCategories.filter(cat => cat.category !== action.categoryName)
        return updateObject(course, { categories: updatedCategories })
      } else {
        return course
      }
...
const reducer = (state = initialState, action) => {
  switch (action.type) {
    case actionTypes.ADD_CATEGORY_TO_COURSE: return addCategoryToCourse(state, action)
    case actionTypes.REMOVE_CATEGORY_FROM_COURSE: return removeCategoryFromCourse(state, action)
    case actionTypes.EDIT_CATEGORY_IN_COURSE: return editCategoryInCourse(state, action)
    case actionTypes.COURSE_SELECTED: return courseSelected(state, action)
    case actionTypes.AUTH_FAIL: return authFail(state, action)
    case actionTypes.AUTH_LOGOUT: return authLogout(state, action)
    case actionTypes.AUTH_SUCCESS: return authSuccess(state, action)
    case actionTypes.SET_DRAWER: return setDrawer(state, action)
